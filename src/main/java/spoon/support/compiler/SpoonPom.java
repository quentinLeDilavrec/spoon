...
public class SpoonPom implements SpoonResource {
	 ...
	 */
	public SpoonPom(String path, MavenLauncher.SOURCE_TYPE sourceType, Environment environment)
	 ...
	 * @throws XmlPullParserException when the file is corrupted
	 */
	public SpoonPom(String path, SpoonPom parent, MavenLauncher.SOURCE_TYPE sourceType, Environment environment)
			throws IOException, XmlPullParserException {
		this.parent = parent;
		...
		try (FileReader reader = new FileReader(pomFile)) {
			...
			throw new IOException("Pom does not exists.", e);
		}
		try {
			for (String module : model.getModules()) {
				addModule(
						new SpoonPom(Paths.get(pomFile.getParent(), module).toString(), this, sourceType, environment));
			}
		} catch (IOException e) {
			throw new IOException("problem with modules", e);
		}
	}
	...
	private void generateClassPathFile(File mvnHome, MavenLauncher.SOURCE_TYPE sourceType, Logger LOGGER,
	 ...
	 *                       ':' It can be an array of file instead of an unique one
	 ...
	 * @param forceRefresh if true forces the invocation of maven to regenerate
	 *                     classpath
	 */
	public String[] buildClassPath(String mvnHome, MavenLauncher.SOURCE_TYPE sourceType, Logger LOGGER,
			boolean forceRefresh) {
		if (mvnHome == null) {
